	 ...
	 */
     ...
     public function queryByPage(){
        $shopName = WSTAddslashes(I('shopName'));
	 	...
	 	$sql = "select g.*,gc.cat_name from sente_goods g 
	 	      left join sente_category gc on g.cat_id=gc.cat_id  
	 	      where  g.isSale=1";
//	 	if($areaId1>0)$sql.=" and p.areaId1=".$areaId1;
//	 	if($areaId2>0)$sql.=" and p.areaId2=".$areaId2;
//	 	if($goodsCatId1>0)$sql.=" and g.goodsCatId1=".$goodsCatId1;
...
//	 	if($goodsName!='')$sql.=" and (g.goodsName like '%".$goodsName."%' or g.goodsSn like '%".$goodsName."%')";
	 ...
	 public function changeGoodsStatus(){
	 	...
	 	foreach($ids as $k=>$id)
	 	{
		 	$this->is_sale = (int)I('status',0);
			$rs = $this->where('goods_id='.$id)->save();
			if(false !== $rs){
				$sql = "select goods_name from sente_goods where goods_id=".$id;
				$goods = $this->query($sql);
				$msg = "";
				if(I('status',0)==0){
					$msg = "商品[".$goods[0]['goods_name']."]已被商城下架";
				}else{
					$msg = "商品[".$goods[0]['goods_name']."]已通过审核";
				}
				$yj_data = array(
					'msgType' => 0,
	 ...
	 public function changeRecomStatus(){
	 	...
	 	$id = I('id',0);
	 	$id = self::formatIn(",", $id);
	 	$this->is_rec = (int)I('status',0);
		$rs = $this->where('goods_id in('.$id.")")->save();
		if(false !== $rs){
			$rd['status'] = 1;
		}
		return $rd;
	 ...
	 public function insert()
     {
         ...
         $data['goods_big_logo'] = $info['savepath'] . $info['savename'];
         $rs = $this->add($data);
         if(false !== $rs){
             $rd['status'] = 1;
         }
