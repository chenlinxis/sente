...
class CategoryModel extends BaseModel {
    /**
	 ...
	 public function insert(){
	 	$rd = array('status'=>-1);
		$data = array();
		$data["cat_name"] = I("catName");
		if($this->checkEmpty($data,true)){
			$data["pid"] = I("parentId",0);
		    $data["is_display"] = (int)I("isShow",0);
			$data["cat_sort"] = (int)I("catSort",0);
			$rs = $this->add($data);
			if($rs){
				$rd['status']= 1;
			}
	 ...
	 public function editName(){
		...
		$data["cat_name"] = I("catName");
	    if($this->checkEmpty($data)){
			$rs = $this->where("cat_id=".$id)->save($data);
			if(false !== $rs){
				$rd['status']= 1;
			}
		}
	  ...
	  */
	  public function queryByList($pid = 0){
	     $rs = $this->where(' pid='.(int)$pid)->select();
		 return $rs;
	  }
	  /**
	   * 获取树形分类
    ...
    public function _getChild($data, $pid, $isClear=false)
    {
	 ...
	 public function editiIsShow(){
		...
		$id = (int)I('id');
	 	...
	 	if($isShow!=1){
	 		//把相关的商品下架了
		 	$sql = "update sente_goods set isShow=0 where cat_id in(".implode(',',$ids).")";
		 	$this->execute($sql);
		 	$sql = "update sente_goods set isShow=0 where cat_id in(".implode(',',$ids).")";
		 	$this->execute($sql);
		 	$sql = "update sente_goods set isShow=0 where cat_id in(".implode(',',$ids).")";
	 	...
	 	$this->is_display = ($isShow==1)?1:0;
	 	$rs = $this->where("cat_id in(".implode(',',$ids).")")->save();
	    if(false !== $rs){
			$rd['status']= 1;
		}
		//发生数据修改,清空分类缓存
	 ...
	 */
	 public function editIsFloor(){
		...
		$id = (int)I('id');
	 	...
	 	//设置该id以及其下级分类是否显示
	 	$rs = $this->where("cat_id in($ids)")->setField('is_tuijian',$isFloor);
