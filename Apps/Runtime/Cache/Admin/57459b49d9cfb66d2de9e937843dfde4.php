...
<html lang="zh-cn">
   ...
   function toggleIsShow(t,v){
	   ...
	   $.post("<?php echo U('Admin/Recruit/editiIsShow');?>",{id:v,isShow:t},function(data,textStatus){
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该文章吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("<?php echo U('Admin/Recruit/del');?>",{id:id},function(data,textStatus){
					var json = WST.toJson(data);
					if(json.status=='1'){
						Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
						location.reload();
   ...
   <body class='wst-page'>
       ...
       <a class="btn btn-success glyphicon glyphicon-plus" href="<?php echo U('Admin/Recruit/toEdit');?>" style='float:right'>新增</a>
       </div>
       </form>
       <div class="wst-body"> 
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               <td><?php echo ($i); ?></td>
               <td><?php echo ($vo['recruit_title']); ?></td>
               <td><?php echo ($vo['recruit_job']); ?></td>
               <td><?php echo ($vo['recruit_dept']); ?></td>
               <td><?php echo ($vo['recruit_type_id']); ?></td>
               <td>
               <div class="dropdown">
                   <div class="dropdown">
